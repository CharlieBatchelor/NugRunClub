...
googleData = pd.read_csv("monthlyData.csv")
# Date Variable - eg 2022-02-01
now = datetime.now()
# Might need a + "0" sometimes? at the beginning
month = str(int(now.strftime("%m"))-1)
stringDate = now.strftime("%Y-") + month
...
googleData.to_csv("monthlyData.csv", index=False)
